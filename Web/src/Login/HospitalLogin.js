...
const HospitalLogin = ({ setLoginType }) => {
  ...
  const Login = () => {
    if (Email === "" || Email == null) {
      ...
      axios.post(MyApiUrl + "HospitalLogin", obj).then((response) => {
        if (response.data === false) {
          alert("Failed To Login, Please Enter Valid Credentials...!");
        } else {
          localStorage.setItem("LoginType", "Hospital");
          localStorage.setItem("HospitalID", response.data[0].hospital_pkid);
        ...
        <div className="container ptb--40">
          <div className="row">
            <div className="breadcrumb-area-wrapper">
              <h1 className="title">Hospital Login</h1>
