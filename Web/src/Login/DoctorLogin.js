...
const DoctorLogin = ({ setLoginType }) => {
  ...
  const Login = () => {
    if (Email === "" || Email == null) {
      ...
      axios.post(MyApiUrl + "DoctorLogin", obj).then((response) => {
        if (response.data === false) {
          alert("Failed To Login, Please Enter Valid Credentials...!");
        } else {
          localStorage.setItem("LoginType", "Doctor");
          localStorage.setItem("DoctorID", response.data[0].doctor_pkid);
          localStorage.setItem(
            "DoctorHospitalID",
            response.data[0].doctor_hospital_id
          );
          setLoginType("Doctor");
